...
public class Calculator {
	 ...
	 * 
	 * 리턴타입 메서드이름(타입 변수명, 타입 변수명, ...){ <-- 여기가 입력 받는 곳!
	 * 		// 메서드 호출시 수행될 코드 
	 * 		return 결과값; <-- 출력하는 곳!
	 * }
	 * 
	...
	public static String divide(int a, int b) {
		return "결과값 : 몫은 " + quotioent(a, b) + ", 나머지는 " + remainder(a, b) ; // 스태틱은 인스턴스 변수 ,메서드 사용 불가능해서 
//	    return "결과값 : 몫은 " + a / b + ", 나머지는 " + a % b ; // 매개변수로 변경 
	}
	
	// 두 수의 몫을 구하는 기능 : quotient
	public static int quotioent(int a, int b) {
		return a / b;
	}
	 ...
	 * - 반복문으로 바꿀 수 잇으며 때때로 반복문보다 성능이 나쁠 때도 있음
	 * - 이해하면간결한 코드 작성 가능
	 * */
	public int factorial3(int n) {
		int result = 1;
		
		if (n != 1) result = n * factorial3(n - 1);
