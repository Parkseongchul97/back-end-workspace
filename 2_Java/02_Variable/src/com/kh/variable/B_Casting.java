...
//		casting.autoCasting();
	 ...
	 * */
	public void casting() {
		double d = 4.123456789012345678901234567890123; // 15자리만 표현
		System.out.println(d);
		
		float f = (float)d; // 6자리만 표현 
		System.out.println(f); // 4.123457 (반올림)
		...
		int sum2 = (int)(i + d); // 4.0 + 4.123456 = 8.123456 에서 자름 
		System.out.println(sum2);
		
		int number = 129; 
		byte bNumber = (byte)number; // -127
		System.out.println(bNumber); // byte -128 ~ 127 129는 2개 넘어가서 -127
		
		// char <-> int : 각 문자들 마다 고유한 숫자가 지정되어 있다. [!-~] 유니코드, 아스키코드
		// 쌍방향으로 형변환 가능 
		...
		char ch = 52345; // 97 = a 52345 = 챹 이런식
