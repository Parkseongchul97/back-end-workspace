 ...
 * 
 * * 스트림의 종료 
 * - java.util.stream 패키지에 존재하고 BasStream 인터페이스를 부모로 갖고
 * 	 자식 인터페이스들이 상속 관계를 이루고 있다.
 * 
 * * Stream API
 * - 스트림은 자바 8부터 추가된 기능으로 커렉션(배열)의 저장 요소들을 하나씩 참조해서
...
public class Create {
	...
	public void method1() {
		...
		int sum = stream.sum();
		System.out.println("range의 sum : " + sum);// 45
		
		// 첫번째 매개값 ~ 두번째 매개 값 까지의 값을 요소로 가지는 스트림 객체 생성 
