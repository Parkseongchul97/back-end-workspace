 ...
 * - 컬렌션을 사용하면 데이터의 추가, 삭제, 정렬 등의 처리가 간단하게 해결되어
...
public class A_ArrayList {
	...
	public void method1() {
		ArrayList<String> list = new ArrayList<String>(); // ArrayList 뒤에<데이터 타입> 지정가능 
...
//		list.add(3);
	...
	public void method2() {
		...
		// 		내부적으로 기존에 있는 것들은 뒤로 밀고 해당 인덱스에 값 추가 
		...
		List<Person> sub = list.subList(0, 2); // 0번째 부터 2번째 까지인데 CSS에서 그리드 처럼 앞으로 자름 
//		List<Person> list2 = new List<Person>(); List는 인터페이스여서 생성 불가
		...
		num /= list.size();
		System.out.println("리스트에 저장된 사람들의 평균 연령 : " + num + "세");
	...
	public void method3() {
		...
		// 12. 내림차순 정렬 : sort 매서드를 통해서 정렬 후 reverse 메서드를 호출하여 역순으로 정렬 (단순 revese는 순서만 바꿔줌)
