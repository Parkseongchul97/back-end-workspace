...
public class A_HashMap {
	...
	public void method2() {
		...
		// 키 값으로 삭제하기 - 꼬깔콘
		map.remove("꼬깔콘");
		System.out.println(map.get("꼬깔콘"));
		...
		int avg=0;
		for(String s : key) { 
			avg += map.get(s).getCalorie();
		}
		System.out.println("총합 : " + avg + "\n평균 : "+ (avg/key.size()));
		int sum= 0;
		// 밖에 key 선언 안하고 안에 keySet 선언해도댐
		for(String k : map.keySet()) {
			Snack snack = map.get(k);
		...
		map.clear();
		System.out.println("비어 있는지? : " + map.isEmpty());
		
	}
	public void method3() {}

	public static void main(String[] args) {
		A_HashMap m = new A_HashMap();
//		m.method1();
		m.method2();
		m.method3();
	}
