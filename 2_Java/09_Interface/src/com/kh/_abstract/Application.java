 ...
 * - 추상 클래스를 상속받는 자식 클래스에서 "반드시" 오버라이딩(재정의) 해야한다.
 * - 오버라이딩(재정의) 해주지 않으면 컴파일 에러 발생 !!
 * - 자식 클래스 에서 강제성을 부여하는 방법 
 * */
...
//		Sports s = new Sports <- 추상 클래스 객체 생성 X 
		Sports b2 = new BasketBall(5);
		Sports f2 =new FootBall(11); // 다형성으로 인해 부모 타입으로 생성 가능! 
		b.rule();
		f2.rule();
