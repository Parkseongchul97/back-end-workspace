 ...
 *  추상클래스와 인터페이스 
 *  공통점
 *  - 객체 생성은 안되나, 참조 변수로는 사용가능하다. (즉, 다형성 적용)
 *  - 상속(구현)하는 클래스에서 추상 메서드를 재정의 하도록 강제한다.(오버라이딩)
 *  
 *  차이점
 ...
 *  
 *  extends와 implements
 *  - 클래스 간의 상속 관계일 떄 : 자식클래스명 extends 부모클래스명
...
//		Vehicle v = new Vehicle(); 인터페이스는 객체 생성 X 
		...
		Tv tv = new Tv();
		tv.turnOn();
		tv.setVolume(-1); // 0 최소값 적용해서
		tv.turnOff();
		
		Audio audio =  new Audio();
		audio.turnOn();
		audio.search("melon.com");
		audio.setVolume(30); // 10 최댓값 지정해서
		audio.turnOff();
	}
	
	/*
	 * 추상 클래스 : 미완성 설계도, 인터페이스 : 기본 설계도 
	 * 개발방법론 : 예전 - 폭포수 (기획 -> 디자인 -> 개발) 
	 * 			 요즘 - 애자일 기획, 디자인, 개발이 반복적으로 거의 동시에 진행 
	 * */
