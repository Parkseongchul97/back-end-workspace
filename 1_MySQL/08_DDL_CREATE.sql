/*
    ...
    - 객체(Obhect)/스키마(Schema) 를 만드고(CREATE), 변경하고(ALTER), 삭제(DROP)하는 언어 
    - 즉, 실제 데이터 값이 아닌 구조 자체를 정의하는 언어 
    
    MySQL 에서 객체(구조) : 테이블(Table), 뷰(View), 인덱스(Index), 프로시저(Procesure), 트리거(Trigger), 함수(Function)
*/
...
-- 테이블에 데이터를 추가 시키는 구문 (DML : INSERT)
...
/*
    ...
    CHECK(조건식)
    - 해당 컬럼에 들어올 수 있는 다른 조건을 제시해볼 수 있음! 
    - 해당 조건에 만족하는 데이터 값만 달릴 수 있음 
*/
...
INSERT INTO mem_unique 
	VALUES(2, 'user01', 'pass02', '주준영', '남', null, null, null);
-- > unique 제약조건에 위배되어 insert 실패 ! 
INSERT INTO mem_unique 
	VALUES(2, 'user02', 'pass02', '주준영', 'ㄴ', null, null, null);
-- >> 성별에 유효한 값이 아니어도 들어감 !

CREATE TABLE mem_cheack(
	mem_no INT NOT NULL,
    mem_id VARCHAR(20) NOT NULL UNIQUE,
    mem_pwd VARCHAR(20) NOT NULL,
    mem_name VARCHAR(20) NOT NULL,
...
SELECT * FROM mem_cheack;

INSERT INTO mem_cheack 
	VALUES(1, 'user01', 'pass01', '장성일', '남', null, null, null);
    
INSERT INTO mem_cheack 
	VALUES(2, 'user02', 'pass02', '이동호', 'ㄴ', null, null, null);
...
/*
    ...
    - PRIMATY KEY 제약조건을 부여하면 그 컬럼에 자동으로 NOT NULL + UNIQUE 조건이 설정 
    - 한 테이블 당 오로지 한 개만 설정 
    --> 간혹 복합키 설정도 가능 
*/
...
INSERT INTO mem_pri
	VALUES(1, 'user02', 'pass02', '배영운', '남' , null, null, null);
-- >> 기본키에 중복값을 담으려고 해서 문제발생 (unique 제약조건 위배)

INSERT INTO mem_pri
	VALUES(null, 'user02', 'pass02', '배영운', '남' , null, null, null);
-- >> 기본키에 null을 담으려고 해서 문제 발생! (not null 제약조건 위배)

INSERT INTO mem_pri
	VALUES(2, 'user02', 'pass02', '배영운', '남' , null, null, null);
    
-- 복합키 사용 예시 (어떤 회원이 어떤 상품을 찜했는지에 대한 데이터를 보관하는 테이블)
-- >> 복합키 방식으로 지정해서 쓰는 건 가급적으로 추천 X  
...
INSERT INTO pro_like 
	VALUES(1, 'B' , current_date());
    
INSERT INTO pro_like 
	VALUES(2, 'B' , current_date());
...
FROM member
    ...
    /*
		...
		--> 다른 테이블을 참조한다고 표현
        --> 주로 FOREIGN KEY 제약조건에 의ㅐ 테이블 간의 관계가 형성됨! 
    */
...
-- >> 외래키 제약조건이 부여되도 기본적으로는 null 허용 
    ...
    INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id)
	VALUES('user03', 'pass03', '우현성', 4);
-- >> 부모키(parent key) 를 찾을 수 없다는 오류발생!! 
...
-- mem_grade 테이블에서 grade_code 가 2인 등급 삭제 
DELETE FROM mem_grade
WHERE grade_code = 2;
-- >> 자식 테이블 (member)에 2 라는 값을 사용하고 있기 때문에 삭제 X 
...
DELETE FROM mem_grade
WHERE grade_code = 3;
-- > 자식 테이블 (member)에 3 이라는 값을 사용하고 있지 않기 때문에 삭제 O

-- 2라는 값을 사용하고 있는 mem_no 4번인 사람을 삭제하면 
...
/*
	...
	2. ON DELETE SET NULL
		부모 데이터 삭제시 해당 데이터를 쓰고 있는 자식 데이터의 값을 NULL 로 처리 
    3. ON DELETE CASCADE
		부모데티어 삭제시 해당 데이터를 쓰고 있는 자식 데이터도 같이 삭제 
*/
...
DELETE FROM mem_grade
WHERE grade_code = 1; 
-- > 삭제 가능! 1을 가져다 쓰고있는 자식 데이터의 grade_id 값이 null로 변경
...
/*
	DEFAULT 기본값
    - 제약조건 아님!
    - 컬럼을 선정하지 않고 INSERT시 NULL 이 아닌 기본값을 세팅해주는 값 
...
SELECT * FROM kh.employee 
WHERE 1 = 0; 
-- 모든 행에 대해서 매번 FALSE 이기 때문에 테이블 구조만 복사!

-- employee 테이블에서 사번, 직원명, 급여 , 연봉만 저장하는 테이블을 employee_copy 테이블로 생성 
CREATE TABLE employee_copy 
SELECT emp_id 사번,
	emp_name 직원명,
    salary 급여,
    (salary * ifnull(bonus, 0))+salary * 12 연봉
FROM kh.employee ;
