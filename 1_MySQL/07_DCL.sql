...
-- 데이터베이스(스키마) 권한 회수 
-- REVOKE ALL PRIVILEGES ON *.* FROM rest;

/*
	데이터베이스(Database) : 데이터의 집합을 저장하고 권리하는 시스템
		- 데이터를 구조화하고 조직화하는데 사용
        - 여러 테이블, 뷰 ,프로시저, 함수 등을 포함
    
    스키마 (Schema) : 데이터 베이스 내에 특정 테이블, 뷰, 프로시저, 함수 등을 그룹화 
    
    --> 데이터베이스와 스키마를 같은 의미로 사용을 하지만, 몇몇 DBMS 에서는 다르게 해석 
        MySQL 에서는 대부분 두 용어를 같이 사용!
*/
-- 스키마 생성
