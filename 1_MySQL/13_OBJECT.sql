/*
	인덱스(INDEX)
    - SQL 명령문의 처리 속도를 향상시키기 위해 핼들의 위치 정보를 가지고 있다. 
...
CREATE INDEX idx_amount -- 비고유 인덱스 생성 
ON sakila.payment(amount); 

CREATE INDEX idx_amount_id -- 결합 인덱스 생성 
...
/*
	프로시저(PROCEDURE)
    - SQL 문을 저장하여 필요할 때마다 다시 입력할 필요 없이 간단하게 호출해서 사용 가능하게 하는 코드 블록 
...
CREATE PROCEDURE del_all_emp(IN id INT) -- 매개변수 넣을때 IN과 데이터 타입 기입 
BEGIN
	DELETE FROM emp_copy
    WHERE emp_id = id ;
END //
...
/*
	함수(FUNCTION)
    - 프로시저와 거의 유하산 용도로 값을 반환하는 재사용 가능한 코드 블록
    ...
    BEGIN
		DECLARE 반환값 자료형
        결과에 해당하는 SQL 문 등 계산식 (result로 처리)
        RETURN 반환값 
    END //
...
-- 방금만든 salary_calc 함수를 이용해서 연봉이 4000만 이상인 사원의 사번 사원명 급여 연봉 조회 

SELECT emp_id 사번,
	emp_name 사원명,
    format(salary, 0) 급여,
    format(salary_calc(emp_id), 0) 연봉
FROM employee
...
DROP FUNCTION IF EXISTS salary_calc; -- IF EXISTS는 존재 할때만 삭제한다는 소리 
...
-- employee 테이블에 새로운 행이 INSERT 될 때마다 '신입사원이 입사했습니다.' 라는 메시지를
-- log 테이블에 자동으로 INSERT 되는 트리거 생성 
...
BEGIN
    ...
    VALUES('UPDATE', '신입사원이 입사했습니다.');
END //

DELIMITER ; 

INSERT INTO employee(emp_id, emp_name, emp_no)
VALUES(880, '서동문', '000000-1234567');



-- employee 테이블에 UPDATE 수행후 업데이트 실행 메세지를 LOG에 담는 trg_2 트리거 생성 


-- OLD : 수정, 삭제 전 데이터에 접근가능
-- NEW : 추가, 수정 후 데이터에 접근가능
...
/*
    ...
    product 테이블에 매번 자동으로 재고수량 UPDATE
*/
/*
DELIMITER //
...
BEGIN
	UPDATE product
	SET stock = if(( prodetail.status) = '입고' , stock + prodetail.amount  , stock - prodetail.amount)
	WHERE pcode = prodetail.pcode ;
END //
