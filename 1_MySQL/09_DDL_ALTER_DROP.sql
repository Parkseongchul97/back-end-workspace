...
/*
	1. 컬럼 추가 / 수정 / 삭제 / 이름변경
    1-1. 컬럼 추가 (ADD)
    - ALTER TABLE 테이블명 ADD 컬럼명 자료형 DEFAULT 기본값; -- 디폴트부터는 생략가능 
*/
-- cname 컬럼 추가 
ALTER TABLE dept_copy ADD cname VARCHAR(20);
-- >> cname 이라는 새로운 컬럼이 만들어지고 기본적으로 null으로 채워짐 
...
/*
	1-2. 컬럼 수정(MODIFY)
    - 자료형 변경 : ALTER TABLE 태이블명 MODIFY 컬럼명 변경할자료형;
    - 기본값 변경 : ALTER TABLE 태이블명 MODIFY 컬럼명 자료형 DEFAULT 변경할기본값;
    
    *참고 : DEFAULT 삭제 
    ALTER TABLE 테이블명 MODIFY 컬럼명 자료형 DEFAULT NULL;
*/
-- dept id 컬럼의 자료형 CHAR(3)으로 변경 
ALTER TABLE dept_copy MODIFY dept_id CHAR(3);

ALTER TABLE dept_copy MODIFY dept_id INT; -- 에러
...
ALTER TABLE dept_copy DROP COLUMN lname; -- 삭제 불가능 (테이블에 남은 마지막 코드이기 때문)
...
-- dept_copy 테이블에 dept_id 컬럼에 기본키 (pk) 제약조건 추가
...
-- 삭제하고 싶다면 자식 테이블 먼저 삭제후 부모 테이블 삭제가능!!
DROP TABLE member;
