/*
    ...
    - DBMS : Database Management System의 약자, 데이터 베이스 관리 시스템
    ...
    3. 데이터의 중복을 최소화할 수 있으며 사용하기 편리하고 데이터의 무결성, 트랜잭션 처리 
	   등 데이터 베이스 관리 시스템으로 뛰어난 성능을 보여준다.
       
	- SQL(Structured Query Language)
    : 관계형 데이터 베이스에서 데이터를 조회하거나 조작하기 위해 사용하는 표준 검색 언어
    
    - SQL 종류
	  1. DML(Data Manipulation Language) : 데이터 조작어 
		- 실제 테이터를 조작하기 위해 사용하는 언어 
        (INSERT : 추가, UPDATE : 수정, DDLETE: 삭제, SELECT)
	  2. DQL(Date Quey Language) : 데이터 질의어
        - 데이터를 조회(검색)하기 위햇 서용하는 언어 (SLECT : 조회)
	  3. DCL(Data Control Language) : 데이터 제어어
        - DBMS에 대한 보안, 무결성, 복구 등 DBMS를 제어하기 위한 언어
          (GRANT : 권한 부여, REVOKE : 권환 회수)
	  4. DDL (Data Definiton Language) : 데이터 정의어
        - DBMS의 구조를 정의하거나 변경 삭제하기 위해 사용하는 언어
        (CREATE : 생성 ALTER : 수정 DROP : 삭제) -테이블 변경과 관련
	  5. TCL(Transactoin Control Language) : 트랜잭션을 제어하는 언어 
...
/*
    ...
    - 조회하고자 하는 컬럼들은 반드시 FROM 절에 기술한 테이블에 존재하는 컬럼이여야 한다.
...
/*
	컬럼 산술 연산 
    - SELECT 절에 컬럼명 입력 부분에 산술 연산자를 사용하여 결과를 조회할 수 있다.
...
/*
	...
	- 별칭을 부여할 때 띄어쓰기 혹은 특수문자가 포함될 경우 반드시 "" 로 감싸준다
*/
SELECT 
	emp_name 직원명,
    salary "직원 월급", 
...
/*
    ...
    - 조회하고자 하나는 테이블로부터 특정 조건에 만족하는 데이터만 조회하고자 할 때 사용
...
-- 2 부서코드가 D1이 아닌 사원들의 사번(emp_id), 사원명 부서코드 조회
...
-- 급여가 350만원 이상이면서 600만원 이하를 받는 사원들의 사원명, 사번, 급여 조회
SELECT
	emp_id,
    emp_name,
    salary
...
-- 입사일(hire_date)이 '1990-01-01' ~ '2001-01-01' *
...
-- 이름의 가운데 글자가 '하'인 사원들의 사원명, 전화번호 조회 (3글자인경우만)
...
-- 예 sun_123@naver.com
-- ESCAPE OPTION : 나만의 와일드 카드를 만들어 사용
...
-- 부서배치를 아직 받지않고 보너스는 받은 사원들의 이름 보너스 부서코드 조회
...
SELECT
    ...
    bonus
FROM employee
WHERE NOT bonus is null AND dept_code is null;
...
SELECT
    ...
    salary
FROM employee
WHERE dept_code IN ('D5','D6','D8'); -- 아래와 같음
...
SELECT 
    ...
    bonus
FROM employee
-- ORDER BY bonus  ; -- 오름차순 정렬 (null이 맨앞)
ORDER BY 2 DESC ; -- 내림차순 정렬 (null이 맨뒤)
-- 숫자는 컬럼의 순서로 2번째 이런식 
...
/*
    ...
    - 데이터 양을 제한하고자 할 때 유용 (페이징 처리)
      {유튜브 같은거 스크롤 내리면 새로 내용 나오는거처럼 그런상황에서 사용} 
...
-- LIMIT 절은 두개의 값이 있을수있음
-- 첫번째 값은 오프셋(offset, 0부터 시작) 시작행을 지정
-- 두번째 값은 반환할 최대 행 수를 지정 
-- 1개의 값만 넣었을시에는 첫번째 값이 자동으로 0으로 생략 
SELECT
	emp_name,
    salary
FROM employee
ORDER BY 2 DESC
LIMIT 7, 10; -- 7번째부터  ~ 10명 
-- LIMIT 10 OFFSET 7; 이런식으로도 표현 가능 

-- 실습문제 ---

-- 1 직급코드 가 J7 이거나 J2인 사원들 중 급여(salary)가 200만원 이상인 사원들의 모든 컬럼 조회
...
--   보너스포함 연봉 : (salary + salary * bonus)* 12
