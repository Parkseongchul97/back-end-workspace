 ...
 * - 추상화된 클래스의 객체(구상객체, 실체클래스 )를 생성하고 반환받는 방법
 * 
 * 1) 추상객체를 실질적으로 생성하는 곳은 싱글톤 패턴으로 작성된 Factory(공장)에서 진행된다.
 * 	  이때 넘어오는 command 값에 따라서 서로 다른 구상 객체가 만들어진다.
...
public class DispatcherServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)  {
		...
		String[] reqestURIList = requestURI.split("/");
		String command = reqestURIList[reqestURIList.length -1];
